...
//   return (
//     <div className="flex items-center gap-4 text-xl lg:text-3xl">
//       {/* Notifications Icon */}
//       {UserData && (
//         <div className="flex items-center gap-4 text-xl lg:text-3xl">
//           <div
...
const Navbar = () => {
  ...
  const {
    ...
    isLoading,
    error,
  } = useQuery(["userData", language], async () => {
        ...
        <div className="flex items-center gap-8 text-sm lg:text-xl font-medium text-gray-700 dark:text-white ps-6">
          {linksToShow.map((link) => (
            <NavLink
              ...
              pathname={pathname}
            >
              {translate(`navbar.${link.text.toLowerCase().replace(" ", "_")}`)}
            </NavLink>
          ))}
        </div>
        <div className="flex justify-around items-center gap-8">
          <SearchInput
            value={searchText}
            onChange={(e) => setSearchText(e.target.value)}
            placeholder={translate("navbar.search_placeholder")}
          />
          <RightSection UserData={UserData} isLoading={isLoading} />
        </div>
      </div>
  ...
  return (
    <div className="absolute top-full left-0 w-full lg:hidden">
      ...
      <motion.div
        ...
        transition={{ duration: 0.4, ease: "easeInOut" }}
      >
        <NavLink href="#">{translate("navbar.find_work")}</NavLink>
        <NavLink href="#">{translate("navbar.my_projects")}</NavLink>
        <NavLink href="#">{translate("navbar.my_proposals")}</NavLink>
        <SearchInput
          value={searchText}
          onChange={(e) => setSearchText(e.target.value)}
          placeholder={translate("navbar.search_placeholder")}
...
const NotificationsDropdown = () => {
  ...
  return (
    ...
    >
      ...
      >
        <IoMdNotificationsOutline className="text-3xl" />
        <span>{translate("navbar.show_all_notifications")}</span>
      </Link>
    </div>
  );
};
...
const MessagesDropdown = () => {
  ...
  return (
    ...
    >
      ...
      >
        <IoChatboxOutline className="text-3xl" />
        {translate("navbar.show_all_messages")}
        <span></span>
      </Link>
    </div>
  );
